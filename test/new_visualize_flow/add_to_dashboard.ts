...
export default function ({ getService, getPageObjects }: FtrProviderContext) {
  ...
  describe.only('Add to Dashboard', function describeIndexTests() {
    before(async () => {
    ...
    it('adding a new area chart to a new dashboard', async function () {
      await PageObjects.visualize.navigateToNewAggBasedVisualization();
      await PageObjects.visualize.clickMetric();
      await find.clickByCssSelector('li.euiListGroupItem:nth-of-type(2)');
      // await PageObjects.visEditor.clickGo();

      await PageObjects.visualize.saveVisualization('My New Vis 1', {
        addToDashboard: true,
      });
