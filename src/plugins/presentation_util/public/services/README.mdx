---
id: presentationUtilServices
slug: /kibana-dev-docs/presentationPlugin/services
title: Plugin Services Toolkit
summary: A collection of objects for creating services local to a plugin.
date: 2020-01-12
tags: ['kibana', 'presentation', 'services']
related: ['presentationUtilPlugin']
---

While Kibana provides a `useKibana` hook for use in a plugin, the number services it provides is very large.  This presents a number of difficulties:
...
<DocAccordion buttonContent="Example" initialIsOpen>
```ts
...
## Creating a `PluginServices` Instance
...
## Creating Registries of Providers
...
A factory is nothing more than a function that uses `StartParameters` to return a set of functions that conforms to a portion of the `Services` specification.  For each service, a factory is provided for each environment.
